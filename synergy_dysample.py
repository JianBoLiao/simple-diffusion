...
class co_dysample(nn.Module):
    def __init__(self, c_in, groups=4, dyscope=True): # writer=None
        ...
        self.register_buffer("last_downkernel", self.Dy_DownSample.offset.weight)
        self.register_buffer("now_upkernel", self.Dy_UpSample.offset.weight)
        self.register_buffer("down_offset", None)

        num_in = self.now_upkernel.numel()
        num_out = self.last_downkernel.numel()
        x1, x2, _, _ = self.last_downkernel.shape
        Rearr = Rearrange('(x1 x2) -> x1 x2 1 1', x1=x1, x2=x2)
        self.Rearr = Rearr
        self.tune = nn.Linear(num_in, num_out)
        constant_init(self.tune, 0)
        self.gate = 0.5

    def down_forward(self, x):
        # if self.last_downkernel is not None:
        # self.last_dow = torch.zeros_like(self.last_downkernel)
        temp1 = self.Dy_DownSample.offset.weight.detach()
        temp2 = self.now_upkernel.flatten()
        tune = self.tune(temp2)

        # print(self.gate(temp2).shape)
        # print(self.last_downkernel.shape)
        # self.writer.add_histogram('self.gate', self.gate)
        self.Dy_DownSample.offset.weight.data = (
                (1 - self.gate) * self.last_downkernel.clone() +
                self.gate * self.Rearr(tune)
        )

        self.last_downkernel = temp1
        x, self.down_offset = self.Dy_DownSample(x)
        # print("down_offset: ", self.down_offset.shape)
        return x

    def up_forward(self, x):
        self.down_offset *= self.c_in / 16
        x = self.Dy_UpSample(x, self.down_offset)
        self.now_upkernel = self.Dy_UpSample.scope.weight  # .detach()
        return x
